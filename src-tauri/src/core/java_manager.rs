...
impl JavaManager {
    ...
    pub fn new() -> Result<Self> {
        ...
        if !config_path.exists() {
            create_dir_all(&config_path).context("No se pudo crear el directorio para las versiones de Java")?;
        }
    ...
    async fn download_java(&self, version: u8, target_dir: &PathBuf) -> Result<()> {
        ...
        if !target_dir.exists() {
            create_dir_all(target_dir).context("No se pudo crear el directorio para la versión de Java")?;
        }
    ...
    fn extract_zip(&self, zip_path: &PathBuf, target_dir: &PathBuf) -> Result<()> {
        ...
        for i in 0..archive.len() {
            let mut file = archive.by_index(i).context("No se pudo acceder al archivo en el ZIP")?;
            ...
            if file.name().ends_with('/') {
                fs::create_dir_all(&outpath).context("No se pudo crear directorio durante la extracción")?;
            } else {
                if let Some(p) = outpath.parent() {
                    if !p.exists() {
                        fs::create_dir_all(p).context("No se pudo crear directorio padre durante la extracción")?;
                    }
                }
                let mut outfile = File::create(&outpath).context("No se pudo crear archivo durante la extracción")?;
                io::copy(&mut file, &mut outfile).context("No se pudo copiar contenido del archivo ZIP")?;
    ...
    fn extract_tar_gz(&self, tar_gz_path: &PathBuf, target_dir: &PathBuf) -> Result<()> {
        ...
        archive.unpack(target_dir).context("No se pudo extraer el archivo tar.gz")?;
    ...
    fn fix_extracted_directory(&self, target_dir: &PathBuf) -> Result<()> {
        ...
        let entries = fs::read_dir(target_dir)
            .context("No se pudo leer el directorio de destino")?;
