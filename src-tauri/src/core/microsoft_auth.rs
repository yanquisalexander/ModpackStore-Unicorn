...
impl MicrosoftAuthenticator {
    ...
    async fn authenticate(client: &reqwest::Client, app_handle: &AppHandle) -> Result<MinecraftAccount, Box<dyn std::error::Error>> {
        // Paso 1: Obtener código de dispositivo
        Self::emit_progress(app_handle, "device_code", "Solicitando código de dispositivo...", 0, None, None);
        ...
        Self::emit_progress(app_handle, "microsoft_token", "Autenticación con Microsoft completada", 30, None, None);
        
        // Paso 3: Autenticar con Xbox Live
        Self::emit_progress(app_handle, "xbox_auth", "Autenticando con Xbox Live...", 40, None, None);
        let xbox_auth_response = Self::authenticate_with_xbox_live(client, &token_response.access_token).await?;
        
        Self::emit_progress(app_handle, "xsts_token", "Obteniendo token XSTS...", 50, None, None);
        let xsts_response = Self::get_xsts_token(client, &xbox_auth_response.Token).await?;
        
        // Paso 4: Autenticar con Minecraft
        Self::emit_progress(app_handle, "minecraft_auth", "Autenticando con Minecraft...", 70, None, None);
        ...
        Self::emit_progress(app_handle, "profile", "Obteniendo perfil de Minecraft...", 90, None, None);
        let profile = Self::get_minecraft_profile(client, &minecraft_token.access_token).await?;
        
        Self::emit_progress(app_handle, "complete", "Autenticación completada con éxito", 100, None, None);
        
        // Calcular tiempo de expiración
        let expiration = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("Time went backwards")
            .as_secs() + minecraft_token.expires_in;
    ...
    async fn get_device_code(client: &reqwest::Client) -> Result<DeviceCodeResponse, Box<dyn std::error::Error>> {
        let params = [
            ("client_id", MICROSOFT_CLIENT_ID),
            ("scope", "XboxLive.signin offline_access"),
        ];
        
        let response = client.post(MICROSOFT_AUTH_URL)
        ...
        if !response.status().is_success() {
            return Err(format!("Error al obtener código de dispositivo: {}", response.status()).into());
        }
    ...
    async fn authenticate_with_xbox_live(
        ...
        let response = client.post(XBOX_AUTH_URL)
    ...
    pub async fn refresh_token(
        ...
        let response = self.client.post(MICROSOFT_TOKEN_URL)
            .header(CONTENT_TYPE, "application/x-www-form-urlencoded")
            .form(&params)
            .send()
            .await?;
